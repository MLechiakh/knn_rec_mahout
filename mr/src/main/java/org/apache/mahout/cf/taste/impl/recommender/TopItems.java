...
public final class TopItems {
	...
	public static long[] getTopItemsIDs(int howMany, LongPrimitiveIterator allItemIDs, IDRescorer rescorer,
			Estimator<Long> estimator) throws TasteException {
		Queue<SimilarItem> topItems = new PriorityQueue<>(howMany + 1, Collections.reverseOrder());
		boolean full = false;
		double lowestTopValue = Double.NEGATIVE_INFINITY;
		while (allItemIDs.hasNext()) {
			...
			double similarity;
			try {
				similarity = estimator.estimate(itemID);
			} catch (NoSuchItemException nsue) {
				continue;
			}
			double rescoredSimilarity = rescorer == null ? similarity : rescorer.rescore(itemID, similarity);
			if (!Double.isNaN(rescoredSimilarity) && (!full || rescoredSimilarity > lowestTopValue)) {
				topItems.add(new SimilarItem(itemID, rescoredSimilarity));
				if (full) {
					topItems.poll();
				} else if (topItems.size() > howMany) {
					full = true;
				...
				lowestTopValue = topItems.peek().getSimilarity();
			}
		}
		int size = topItems.size();
		if (size == 0) {
		...
		List<SimilarItem> sorted = new ArrayList<>(size);
		sorted.addAll(topItems);
		Collections.sort(sorted);
		long[] result = new long[size];
		int i = 0;
		for (SimilarItem similarItem : sorted) {
			result[i++] = similarItem.getItemID();
		}
		return result;
	}

	public static List<Bicluster<Long>> getTopBiclusters(int howMany, Iterator<Bicluster<Long>> biclusters,
			Estimator<Bicluster<Long>> estimator) throws TasteException {
		int biclusterID = 0;
	...
	/**
	 * <p>
	 * Thanks to tsmorton for suggesting this functionality and writing part of the
	 * code.
	 * </p>
	 * 
	...
	public static List<GenericItemSimilarity.ItemItemSimilarity> getTopItemItemSimilarities(int howMany,
		...
		Queue<GenericItemSimilarity.ItemItemSimilarity> topSimilarities = new PriorityQueue<>(howMany + 1,
	...
	public static List<GenericUserSimilarity.UserUserSimilarity> getTopUserUserSimilarities(int howMany,
		...
		Queue<GenericUserSimilarity.UserUserSimilarity> topSimilarities = new PriorityQueue<>(howMany + 1,
				Collections.reverseOrder());
		boolean full = false;
		double lowestTopValue = Double.NEGATIVE_INFINITY;
		while (allSimilarities.hasNext()) {
