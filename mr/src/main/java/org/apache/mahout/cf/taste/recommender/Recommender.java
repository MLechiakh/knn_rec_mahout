 ...
 * Implementations will likely take advantage of several classes in other
 * packages here to compute this.
 * </p>
 */
public interface Recommender extends Refreshable {
	 ...
	 * @return {@link List} of recommended {@link RecommendedItem}s, ordered from
	 *         most strongly recommend to least
	 * @throws TasteException if an error occurs while accessing the
	 *                        {@link DataModel}
	 */
	 ...
	 * @return {@link List} of recommended {@link RecommendedItem}s, ordered from
	 *         most strongly recommend to least
	 * @param includeKnownItems whether to include items already known by the user
	 *                          in recommendations
	 * @throws TasteException if an error occurs while accessing the
	 *                        {@link DataModel}
	 */
	 ...
	 * @param userID   user for which recommendations are to be computed
	 * @param howMany  desired number of recommendations
	 * @param rescorer rescoring function to apply before final list of
	...
	FastIDSet getCandidateItems(long userID) throws TasteException;

	/**
	 * @param userID user ID whose preference is to be estimated
	 * @param itemID item ID to estimate preference for
	 * @return an estimated preference if the user has not expressed a preference
	 *         for the item, or else the user's actual preference for the item. If a
	 *         preference cannot be estimated, returns {@link Double#NaN}
	 * @throws TasteException if an error occurs while accessing the
	 *                        {@link DataModel}
	 */
	...
	/**
	 * @return underlying {@link DataModel} used by this {@link Recommender}
	 *         implementation
	 */
